...
PoseEstimator::PoseEstimator(
  ...
  mean.middleRows(6, 4) =
      Eigen::Vector4f(quat.w(), quat.x(), quat.y(), quat.z()).normalized();
...
void PoseEstimator::predict(const rclcpp::Time &stamp) {
  ...
  if ((stamp - init_stamp).seconds() < cool_time_duration ||
      prev_stamp.seconds() == 0.0 || prev_stamp == stamp) {
    prev_stamp = stamp;
    return;
  }
...
void PoseEstimator::predict_odom(const Eigen::Matrix4f &odom_delta) {
  if (!odom_ukf) {
    ...
    Eigen::MatrixXf odom_measurement_noise =
        Eigen::MatrixXf::Identity(7, 7) * 1e-3;

    Eigen::VectorXf odom_mean(7);
    odom_mean.block<3, 1>(0, 0) =
        Eigen::Vector3f(ukf->mean[0], ukf->mean[1], ukf->mean[2]);
...
PoseEstimator::correct(const rclcpp::Time                      &stamp,
  ...
  if (!odom_ukf) {
    ...
    init_guess.block<3, 3>(0, 0) =
        Eigen::Quaternionf(fused_mean[3], fused_mean[4], fused_mean[5],
                           fused_mean[6])
            .normalized()
            .toRotationMatrix();
  }
  ...
  if (odom_ukf) {
    ...
    odom_pred_error =
        odom_guess.inverse() * registration->getFinalTransformation();
  }
...
const boost::optional<Eigen::Matrix4f> &
PoseEstimator::wo_prediction_error() const {
  return wo_pred_error;
}

const boost::optional<Eigen::Matrix4f> &
PoseEstimator::imu_prediction_error() const {
  return imu_pred_error;
}

const boost::optional<Eigen::Matrix4f> &
PoseEstimator::odom_prediction_error() const {
